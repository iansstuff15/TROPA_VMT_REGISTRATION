...
export default async function handler(req, res) {
  ...
  const  spreadsheetId = process.env.SHEETS_ID;
   await googleSheets.spreadsheets.values.append({
       auth,
       spreadsheetId,
       range:'Donations_Reciept_In_Kind!A2',
       valueInputOption: 'USER_ENTERED',
       resource: {
           values:[[
           body.RID, body.firstName,body.lastName,body.email,body.phone,body.received_by, body.date_recieved, now
            ]]
       }
   })
   await googleSheets.spreadsheets.values.append({
    auth,
    spreadsheetId,
    range:'Public_In_Kind_Donations_Data!A2',
    valueInputOption: 'USER_ENTERED',
    resource: {
        values:[[
        body.RID,  body.date_recieved, now
         ]]
    }
})
